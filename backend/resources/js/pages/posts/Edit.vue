...
import { index as postsIndex, show as postsShow, preview as postsPreview } from '@/routes/posts';
...
const props = defineProps<{
    post: any;
    categories: any[];
    posts: any[]; // Existing posts for related posts selection (excluding current post)
}>();
...
const form = useForm({
    ...
    scheduled_at: props.post.scheduled_at ? props.post.scheduled_at.substring(0, 16) : '', // format to YYYY-MM-DDThh:mm
    cover_image_id: props.post.cover_image_id || null,
...
// Sync relationships from props on load
onMounted(() => {
    if (props.post.categories) {
        form.category_ids = props.post.categories.map((c: any) => c.id);
    }
    if (props.post.related_posts) {
        form.related_post_ids = props.post.related_posts.map((rp: any) => rp.id);
    }
    if (props.post.cover_image) {
        selectedCoverMedia.value = props.post.cover_image;
    }
});
...
const isSlugManuallyEdited = ref(true); // Don't auto-overwrite slug on Edit by default
...
const submit = () => {
    form.put(`/admin-cms/posts/${props.post.id}`, {
        onSuccess: () => {
            // Handled
        }
    });
                            ...
                            <div class="border border-sidebar-border/70 rounded-xl p-4 bg-white dark:bg-[#202124] shadow-xs text-left">
                                ...
                                <p class="text-xs text-[#4d5156] dark:text-[#bdc1c6] font-sans mt-1.5 leading-relaxed line-clamp-3">
                                    <span class="text-[#70757a] dark:text-[#9aa0a6] text-xs font-normal">
                                        {{ new Date(props.post.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) }} — 
                                    </span>
                                    {{ seoDescPreview }}
                                </p>
                            </div>
