...
const form = useForm({
    ...
    category_id: props.project.category_id as string | number,
    ...
    key_features: (props.project.key_features || []) as Array<{
        title: string;
        description: string;
        icon: string;
    }>,
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/70 pb-4">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-bold tracking-tight text-foreground">Edit Proyek</h1>
                        <span
                            class="text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase border"
                            :class="project.status === 'published' ? 'bg-emerald-500/15 text-emerald-600 border-emerald-500/30 dark:text-emerald-400' : 'bg-slate-500/10 text-slate-600 border-slate-500/20 dark:text-slate-400'"
                        >
                            {{ project.status }}
                        </span>
                    </div>
            ...
            <div class="flex items-center gap-2">
                <a :href="projectsShow({ project: project.id }).url" target="_blank">
                    <Button type="button" variant="outline" class="h-9 text-xs flex items-center gap-1.5 cursor-pointer border-border/80">
