    ...
    <div ref="heroSection" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-10 opacity-0 -translate-y-3">
      <div class="space-y-4 max-w-3xl">
        ...
        <div class="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/30 shadow-xs">
          ...
          <span class="text-[10px] font-bold tracking-wider text-brand-purple dark:text-indigo-400 uppercase">Kategori Blog</span>
        </div>

        <!-- Heading -->
        <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight leading-[1.2] text-zinc-900 dark:text-white">
          Artikel <span class="bg-gradient-to-r from-brand-purple to-brand-green bg-clip-text text-transparent">{{ category?.name || '' }}</span>
        </h1>
        ...
        <p class="text-sm md:text-base text-zinc-500 dark:text-zinc-400 leading-relaxed font-light mt-2">
          {{ category?.description || 'Kumpulan artikel dan tutorial dalam kategori ini.' }}
        </p>
      </div>
    </div>
    ...
    <div ref="filterSection" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 opacity-0 translate-y-3 relative z-30">
      <div class="flex flex-col lg:flex-row gap-6 lg:items-center justify-between pb-6 border-b border-zinc-200/50 dark:border-zinc-800/40">
        ...
        <div class="flex items-center gap-2.5">
          <NuxtLink 
            to="/blog"
            class="px-5 py-2.5 rounded-full text-xs font-semibold tracking-wide bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 border border-zinc-200/80 dark:border-zinc-800/80 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all duration-300 shadow-sm flex items-center gap-2"
          >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 xl:col-span-9 space-y-12">
          ...
          <div v-else class="text-center py-20 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl">
            ...
            <h4 class="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mt-4">Belum ada artikel</h4>
...
const { data: categoryResponse, error: categoryError } = await useFetchAPI<any>(`/blog-categories/${slug}`)

// Handle 404 if category not found
if (categoryError.value || !categoryResponse.value?.data) {
  showError({
    statusCode: 404,
    statusMessage: 'Kategori blog tidak ditemukan.'
  })
}
...
useSeoMeta({
  title: () => `Artikel ${category.value?.name || 'Kategori'}`,
  description: () => category.value?.description || 'Kumpulan artikel dan tutorial berdasarkan kategori.',
  ogTitle: () => `Artikel ${category.value?.name || 'Kategori'} | growthcoder.id`,
  ogDescription: () => category.value?.description || 'Kumpulan artikel dan tutorial berdasarkan kategori.',
...
useHead(() => ({
  link: [
    { rel: 'canonical', href: `https://growthcoder.id/blog/kategori/${slug}` }
  ],
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        '@type': 'CollectionPage',
        'name': `Artikel ${category.value?.name || ''}`,
        'description': category.value?.description || '',
        'url': `https://growthcoder.id/blog/kategori/${slug}`
      })
    }
  ]
}))
...
const getCategoryColorClass = (categorySlug: string) => {
  ...
  return colorMap[categorySlug] || '!bg-zinc-700 !text-white'
}
