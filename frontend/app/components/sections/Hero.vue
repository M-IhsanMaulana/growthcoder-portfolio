<template>
  <section
    class="relative w-full min-h-[calc(100vh-64px)] flex flex-col justify-center items-center overflow-hidden grid-bg-subtle py-12 lg:py-0">
    <!-- Background Radial Glows -->
    <div
      class="absolute top-[20%] left-[20%] w-[350px] h-[350px] rounded-full bg-purple-600/10 dark:bg-purple-600/15 blur-[100px] pointer-events-none z-0">
    </div>
    <div
      class="absolute bottom-[20%] right-[15%] w-[450px] h-[450px] rounded-full bg-blue-600/10 dark:bg-blue-600/15 blur-[120px] pointer-events-none z-0">
    </div>
    ...
    <div
      class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8">
      <!-- Left Column: Content -->
      <div class="max-w-2xl flex-1 space-y-8 text-left">
        <!-- Availability Badge -->
        <div class="animate-left-item opacity-0">
          <span
            class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-medium bg-emerald-500/5 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
        ...
        <!-- Large Headline -->
        <h1
          class="animate-left-item opacity-0 text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.1] text-zinc-900 dark:text-white">
          ...
          <template v-else>
            ...
            <span class="text-blue-purple-gradient">{{ settings?.owner_title || 'Software Developer' }}</span>
          </template>
        </h1>
        ...
        <div class="animate-left-item opacity-0 flex flex-wrap items-center gap-4">
          <NuxtLink :to="settings?.hero_cta_url || '/proyek'"
        ...
        <div class="animate-left-item opacity-0 flex items-center space-x-5 pt-2">
          <!-- GitHub -->
          <a v-if="settingsPending || settings?.social_github" :href="settings?.social_github || '#'" target="_blank"
          ...
          <!-- LinkedIn -->
          <a v-if="settingsPending || settings?.social_linkedin" :href="settings?.social_linkedin || '#'"
          ...
          <!-- Email -->
          <a v-if="settingsPending || settings?.contact_email"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-4 border-t border-zinc-100 dark:border-zinc-900">
          ...
          <div class="animate-stat-item opacity-0">
            ...
            <template v-else>
              ...
              <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 uppercase tracking-wider font-semibold">Projects
              </p>
            </template>
          </div>
          <!-- Technologies stat -->
          <div class="animate-stat-item opacity-0">
            ...
            <template v-else>
              ...
              <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 uppercase tracking-wider font-semibold">
                Technologies</p>
            </template>
          </div>
          <!-- Extra stats from about_stats (Years Learning, Passion) -->
          <div v-for="(stat, index) in extraStats" :key="index" class="animate-stat-item opacity-0">
            <template v-if="statsPending">
              <Skeleton width="4rem" height="2.25rem" class="mb-1" />
      ...
      <div class="flex-1 flex items-center justify-center relative w-full min-h-[420px] lg:min-h-[500px]">
        ...
        <div
          class="absolute w-80 h-80 rounded-full bg-gradient-to-tr from-purple-500 to-blue-500 opacity-25 blur-[120px] pointer-events-none">
        </div>

        <!-- Programming Illustration (No Box, No Code Editor) -->
        <div
          class="animate-portrait-card opacity-0 relative w-full max-w-[340px] sm:max-w-[450px] lg:max-w-[490px] flex items-center justify-center z-10">
          <NuxtImg src="/programming.svg" alt="Programming Illustration"
        ...
        <div class="animate-float-badge opacity-0 absolute top-[10%] left-[2%] sm:left-[8%] z-20 animate-float-1">
          <div
            class="glass-card-premium rounded-2xl p-2.5 flex items-center justify-center shadow-lg border border-zinc-200/30 dark:border-white/5 w-12 h-12"
            title="Laravel">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-6 h-6 fill-[#FF2D20]">
              <path
        ...
        <div class="animate-float-badge opacity-0 absolute top-[5%] right-[5%] sm:right-[10%] z-20 animate-float-2">
          <div
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 48 48"
              class="fill-zinc-800 dark:fill-white transition-colors duration-300">
              <path
        ...
        <!-- Vue Badge -->
        <div
          class="animate-float-badge opacity-0 absolute bottom-[20%] left-[-15px] sm:left-[0px] z-20 animate-float-3">
          <div
            class="glass-card-premium rounded-2xl p-2.5 flex items-center justify-center shadow-lg border border-zinc-200/30 dark:border-white/5 w-12 h-12"
            title="Vue.js">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 48 48">
        ...
        <div class="animate-float-badge opacity-0 absolute bottom-[10%] right-[5%] sm:right-[12%] z-20 animate-float-1">
          <div
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"
              class="w-6 h-6 fill-zinc-800 dark:fill-white transition-colors duration-300">
              <path
